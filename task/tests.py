...
def test_list_tasks(api_client, create_user):
    ...
    Task.objects.create(project=project, title="T1", description="D1", status="todo", priority="low", assigned=dev, due_date="2025-12-31")
    Task.objects.create(project=project, title="T2", description="D2", status="in_progress", priority="high", assigned=dev, due_date=timezone.make_aware(datetime.datetime(2025, 12, 31)))
...
def test_update_task(api_client, create_user):
    ...
    task = Task.objects.create(
        ...
        priority="medium",
        assigned=dev,
        due_date=timezone.make_aware(datetime.datetime(2025, 12, 31))
    )
...
def test_delete_task(api_client, create_user):
    ...
    dev, _ = create_user(RoleChoices.DEVELOPER)
    project = Project.objects.create(title="P1", description="Test", deadline=timezone.make_aware(datetime.datetime(2025, 12, 31)), owner=manager)
    task = Task.objects.create(project=project, title="DeleteMe", description="Test", status="todo", priority="low", assigned=dev, due_date=timezone.make_aware(datetime.datetime(2025, 12, 31)))
